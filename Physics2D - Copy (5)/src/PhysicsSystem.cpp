...
bool IsStandingOnSlope(int i, int slopeID, const PhysicsWorld& world) {
    ...
    return feetY <= slopeSurfaceY + 8.0f;
}
} // namespace
...
int PhysicsSystem::FindFloorMultiTile(int entityIdx, const PhysicsWorld& world, SpatialGrid& grid) {
    ...
    for (int otherID : neighbors) {
        ...
        if (!(sMaxY > tileMinY && sMinY < tileMaxY)) continue;
        ...
        if (overlapX < entityWidth * 0.4f) continue;
...
void PhysicsSystem::ResolveParticle(int i, PhysicsContext& ctx) {
    ...
    for (int otherID : neighbors) {
        ...
        if (world.slopeType[otherID] != SlopeType::NONE) {
            ...
            float feetY = world.posY[i] + world.size[i].y * 0.5f;
            if (feetY >= slopeSurfaceY - 0.5f && world.velY[i] >= 0.0f) {
                world.posY[i] = slopeSurfaceY - (world.size[i].y * 0.5f);
                world.velY[i] = 0.0f;
...
void PhysicsSystem::UpdateChunk(int start, int end, float dt, PhysicsContext& ctx, bool gridMovement) {
    ...
    for (int i = start; i < end; ++i) {
        ...
        // isGhosted already means "invisible to every OTHER row's neighbor query" (see
        // ResolveParticle's own isGhosted[otherID] check) -- without this, a ghosted row was still
        // fully processed as ITSELF (gravity/position integration, and its own ResolveParticle call
        // querying its OWN neighbors), so its onCollision callback kept firing every single frame
        // anything stayed overlapping it, even though it was already "dead" gameplay-wise. Ghosted
        // needs to mean fully inert on both sides of a collision check, not just as a target.
        if (world.isGhosted[i]) continue;
