...
void PhysicsSystem::PlatformPhysics(int i, PhysicsContext& ctx, float dt) {
    ...
    if (ctx.entities.parentEntity[slot] != -1) {
        ...
        if (PlatformSensor(i, world, ctx.grid) == -1) {
            ...
            // Only apply the RIDER's own intended movement here -- the platform's own motion
            // (master velocity-driven, OR a slave's broadcast reposition in UpdatePlatforms,
            // which never touches velX) is carried separately via the position-delta carry in
            // the caller's per-frame Update (comparing posX/prevPos before/after UpdatePlatforms
            // runs). Adding `world.velX[p] * dt` here on top of that double-counted the
            // platform's movement for anyone riding a platform whose velX is actually driven
            // (the group master), making platform-riding move much faster than walking anywhere
            // else, while a slave-rider (velX never set by the broadcast step) relied entirely on
            // the external carry and felt correct -- inconsistent depending on which segment you
            // stood on.
            float moveX = world.velX[i] * dt;
