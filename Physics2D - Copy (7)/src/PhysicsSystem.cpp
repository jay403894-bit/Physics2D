...
void PhysicsSystem::ResolveParticle(int i, PhysicsContext& ctx) {
    ...
    // Callback iteration -- no neighbor list materialized. This ran once per row per
    // substep and was the hottest heap-allocation site in the engine (a fresh
    // std::vector per call, across every worker thread). `continue` in the old loop
    // is `return` in the lambda.
    ctx.grid.forEachCandidate(i, world.posX[i], world.posY[i], world.size[i].x, world.size[i].y,
        [&](int otherID) {
        if (i == otherID) return;
